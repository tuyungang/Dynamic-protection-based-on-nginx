...
//extern ngx_socket_t ngx_write_mychannel;
/*
static char *url_tags[7] =
{
    "<link",
    "<img",
    "<form",
...
/*add by tu*/
//static ngx_int_t ngx_http_myproxy_encrypt_url(ngx_event_pipe_t *p, u_char *buf_pos, ssize_t buf_size, ngx_buf_t *b, ssize_t *count, ngx_int_t *flag);
static ngx_int_t ngx_http_myproxy_decrypt_url(ngx_http_request_t *r, u_char *buf_pos, size_t buf_size, u_char *buf_out, size_t *buf_out_len);
//static ngx_int_t ngx_check_request_validation(ngx_http_request_t *r);
//static ngx_int_t ngx_myproxy_des_encrypt(ngx_event_pipe_t *p, u_char *key, u_char *buf_pos, ssize_t buf_size, ngx_buf_t *b, ssize_t *count, ngx_int_t *flag);
//static int mergeArrayToInt(u_char *key); 
//ngx_int_t ngx_http_myproxy_check_backlist(ngx_http_request_t *r, u_char *in_buf, size_t buf_len);
/*add by tu*/
...
static ngx_command_t  ngx_http_myproxy_commands[] = {
    ...
    { ngx_string("myproxy_cache_max_range_offset"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
    ...
    { ngx_string("myproxy_cache_background_update"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG,
      ngx_conf_set_flag_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
...
/*
static ngx_int_t 
ngx_check_request_validation(ngx_http_request_t *r)
{
    return NGX_OK;
}
*/
...
ngx_int_t ngx_http_myproxy_check_backlist(ngx_http_request_t *r, u_char *in_buf, size_t buf_len)
{
    printf("enter ngx_http_myproxy_check_backlist\n");
    printf("cookie:%s\n", in_buf);
    ngx_black_t *it = NULL;
    ngx_threadpool_t *pool = threadpool_ptr;
    if (pthread_mutex_lock(&pool->m_BlackMutex) != 0){
        return NGX_OK;
    }
    TAILQ_FOREACH(it, &pool->m_BlackQueue, m_BlackNext) {
        if (ngx_memcmp(it->m_Cookie, in_buf, buf_len) == 0)
            break;
    }
    if (it == TAILQ_END(&pool->m_BlackQueue)) {
        pthread_mutex_unlock(&pool->m_BlackMutex);
        printf("0 quit ngx_http_myproxy_check_backlist\n");
        return NGX_OK;
    }
    pthread_mutex_unlock(&pool->m_BlackMutex);
    printf("1 quit ngx_http_myproxy_check_backlist\n");
...
static ngx_int_t 
ngx_http_myproxy_decrypt_url(ngx_http_request_t *r, u_char *buf_pos, size_t buf_size, u_char *buf_out, size_t *buf_out_len)
{
    int rc;

    //u_char key[20] = {0};
    //ngx_memcpy(key, r->connection->addr_text.data, r->connection->addr_text.len);

    u_char tmp[1024] = {0};

    ngx_memcpy(tmp, buf_pos, buf_size);
    int len = 0;
    char tmp1[1024] = {0};

    //TODO:decrypt
    rc = base64_decode((char*)tmp, (int)buf_size, tmp1, &len);
    if (rc != 0) {
        return NGX_ERROR;
    }
    if (ngx_strncmp(tmp1, "https://", ngx_strlen("https://")) == 0) {
        u_char *b = (u_char*)ngx_strstr(tmp1 + (ngx_strlen("https://") + 1), "/");
        size_t size = len - (b - (u_char*)tmp1);
        ngx_memcpy(buf_out, b, size);
        *buf_out_len = size;

    } else {
        ngx_memcpy(buf_out, tmp1, len);
        *buf_out_len = (size_t)len;
    }
...
ngx_http_proxy_create_request(ngx_http_request_t *r)
{
    ...
    *b->last++ = ' ';
    ...
    if (plcf->proxy_lengths && ctx->vars.uri.len) {
        ...
        ngx_memset(task->m_TaskBuf, '\0', MAX_TASKBUF_SIZE);
        task->handler = ngx_send_data_via_mq_handler;
        */
        /*
        ...
        */
        ...
        if ((spec_flag = (u_char*)ngx_strstr(r->unparsed_uri.data, "10110010")) == NULL)
            goto END_URL; 

        /*
        if (*(r->unparsed_uri.data + (spec_flag - r->unparsed_uri.data - 1)) != '/' ) {
            goto END_URL;
        }
        */

        u_char temp_buffer[5];
        ngx_memset(temp_buffer, '\0', 5);
        ngx_memcpy(temp_buffer, spec_flag + ngx_strlen("10110010"), 4);
        ngx_int_t url_size;
        if (temp_buffer[0] == '0') {
            if (temp_buffer[1] == '0') {
                if (temp_buffer[2] == '0') {
                    url_size = ngx_atoi(temp_buffer + 3, 1);
                } else {
                    url_size = ngx_atoi(temp_buffer + 2, 2);
                }
            } else {
                url_size = ngx_atoi(temp_buffer + 1, 3);
            }
        } else {
            url_size = ngx_atoi(temp_buffer, 4);
        }

        //ngx_memcpy(&url_size, spec_flag + ngx_strlen("10110010"), sizeof(size_t));
        u_char *temp_pos = (u_char*)ngx_strstr(r->uri.data, " HTTP/1.1");
        u_char *url_pos = spec_flag + (ngx_strlen("10110010") + 4);
        size_t cmp_size = temp_pos - url_pos;

        //u_char key[20] = {0};
        //ngx_memcpy(key, r->connection->addr_text.data, r->connection->addr_text.len);
        
        //TODO:decrypt
        ngx_http_myproxy_decrypt_url(r, url_pos, (size_t)url_size, dec_out, &nSize);
        b->last = ngx_copy(b->last, dec_out, nSize);
        if (url_size != (int)cmp_size) {
            b->last = ngx_copy(b->last, url_pos + url_size, cmp_size - url_size);
        }
        ...
        /*
        time_t tm = time(0);
        ngx_memcpy(task->m_TaskBuf + off_x, &tm, sizeof(time_t));
        off_x += sizeof(time_t);
        ngx_memcpy(task->m_TaskBuf + off_x, &nSize, sizeof(size_t));
        off_x += sizeof(size_t);
        ngx_memcpy(task->m_TaskBuf + off_x, dec_out, nSize);
        off_x += nSize;
        size_t tlen = ngx_strlen(client_ip);
        ngx_memcpy(task->m_TaskBuf + off_x, &tlen, sizeof(size_t));
        off_x += sizeof(size_t);
        ngx_memcpy(task->m_TaskBuf + off_x, client_ip, ngx_strlen(client_ip));
        off_x += ngx_strlen(client_ip);
        ngx_memcpy(task->m_TaskBuf + off_x, &client_port, sizeof(unsigned short));
        off_x += sizeof(unsigned short);
        tlen = r->method_name.len;
        ngx_memcpy(task->m_TaskBuf + off_x, &tlen, sizeof(size_t));
        off_x += sizeof(size_t);
        ngx_memcpy(task->m_TaskBuf + off_x, r->method_name.data, r->method_name.len);
        off_x += r->method_name.len;
        tlen = r->http_protocol.len;
        ngx_memcpy(task->m_TaskBuf + off_x, &tlen, sizeof(size_t));
        off_x += sizeof(size_t);
        ngx_memcpy(task->m_TaskBuf + off_x, r->http_protocol.data, r->http_protocol.len);
        off_x += r->http_protocol.len;
        */

        /*
        ngx_int_t rc;
        ngx_array_t *a = &(r->headers_in.cookies);
        ngx_uint_t n = a->nelts;
        ngx_table_elt_t **h = a->elts; 
        ngx_uint_t i;
        ngx_str_t temp;
        ngx_str_null(&temp);
        size_t nlen = 0 ;
        u_char *p = NULL, *end = NULL;
        for (i = 0; i < n; i++) {
            if (h[i]->hash == 0) {
                continue;
            }
            nlen += h[i]->value.len + 2;
        }
        if (nlen == 0) {
            tlen = 1;
            ngx_memcpy(task->m_TaskBuf + off_x, &tlen, sizeof(size_t));
            off_x += sizeof(size_t);
            ngx_memcpy(task->m_TaskBuf + off_x, "0", ngx_strlen("0"));
            off_x += ngx_strlen("0");
            goto SEND_MQ;
        }
        nlen -= 2;
        if (n == 1) {
            temp.data = (*h)->value.data;
            temp.len = (*h)->value.len;
            tlen = temp.len;
            ngx_memcpy(task->m_TaskBuf + off_x, &tlen, sizeof(size_t));
            off_x += sizeof(size_t);
            ngx_memcpy(task->m_TaskBuf + off_x, temp.data, temp.len);
            off_x += temp.len;
            goto SEND_MQ;
        }
        p = ngx_pnalloc(r->pool, nlen);
        if (p == NULL) {
            return NGX_ERROR;
        }
        temp.data = p;
...
SEND_MQ:
        rc = ngx_http_myproxy_check_backlist(r, temp.data, temp.len);
        if (rc != NGX_OK) {
            printf("ngx_http_filter_finalize_request\n");
            rc = ngx_http_filter_finalize_request(r, NULL, NGX_HTTP_MOVED_TEMPORARILY);
            switch (rc) {
                case NGX_OK:
                case NGX_DONE:
                    free(task);
...
FINAL_END:
        ...
        if (r->valid_location) {
            ...
            if (((u_char*)ngx_strstr(r->uri.data + loc_len, "10110010")) == NULL)
                b->last = ngx_copy(b->last, ctx->vars.uri.data, ctx->vars.uri.len);
        }
        ...
        if (escape) {
            ...
            */
            ...
            if ((spec_flag = (u_char*)ngx_strstr(r->uri.data + loc_len, "10110010")) == NULL)
                goto END_TWO_URL; 

            /*
            if (*(r->uri.data + (spec_flag - r->uri.data - 1)) != '/' ) {
                goto END_TWO_URL; 
            }
            */
...
{
    u_char ks[20] = {0};
    ngx_memcpy(ks, key, ngx_strlen(key));
    char *p = NULL;
    int i = 4;
    int array[4];
    for (p = strtok((char*)ks, "."); p != NULL; p = strtok(NULL, ".")) {
        array[--i] = atoi(p);
    }
...
/*add by tu*/
/*
static ngx_int_t ngx_3des_encrypt(ngx_http_request_t *r, u_char *u_key, u_char *url_pos, ssize_t url_size, ngx_buf_t *b, ssize_t *count, ngx_int_t *flag)
{
    int i = 0;
    int len = 0;
    int nlen = 0;
    ...
    //ByteToHexStr(out, dst, ngx_strlen(out));

    ngx_buf_t *tmp_buf;
    ssize_t nSize = b->last - (url_pos + url_size);
    tmp_buf = ngx_create_temp_buf(r->pool, nSize);
    ngx_memcpy(tmp_buf->pos, url_pos + url_size, nSize);

    //ngx_memcpy(url_pos, out, ngx_strlen(out));
    //ngx_memcpy(url_pos + ngx_strlen(out), tmp_buf->pos, nSize);

    //for (i = 0; i < len; i += 8) {
    //    DES_ecb3_encrypt((const_DES_cblock *)(url_pos + i), (DES_cblock *)(tmp + i), &ks1, &ks2, &ks3, DES_DECRYPT);
    //}
    //ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "len1:");
    //ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, (char*)tmp);

    //u_char *pos = (u_char*)ngx_strstr(url_pos, "\"");
    //ssize_t tmp_size = pos - url_pos;
    //memset(out, 0, 1024);
    //
    //ByteToHexStr(out, dst, ngx_strlen(out));

    for (i = 0; i < (int)ngx_strlen(out); i += 8) {
        DES_ecb3_encrypt((const_DES_cblock *)(out + i), (DES_cblock *)(tmp1 + i), &ks1, &ks2, &ks3, DES_DECRYPT);
    }

    ngx_memcpy(url_pos, out, ngx_strlen(out));
    ngx_memcpy(url_pos + ngx_strlen(out), tmp_buf->pos, nSize);
    //b->last = url_pos + (ngx_strlen(out) + nSize);

    ssize_t tmp_size;
    if ((tmp_size = ngx_strlen(out)) > url_size) {
        *count = tmp_size - url_size;
        *flag = 1;

    } else if ((tmp_size = ngx_strlen(out)) < url_size) {
        *count = url_size - tmp_size;
        *flag = -1;
...
static ngx_int_t ngx_myproxy_des_encrypt(ngx_event_pipe_t *p, u_char *key, u_char *buf_pos, ssize_t buf_size, ngx_buf_t *b, ssize_t *count, ngx_int_t *flag)
{
    ...
    //ngx_3des_encrypt(r, (u_char*)"192.168.2.155qwertyuiopa", buf_pos, buf_size, b, count, flag);

    int keyInt;
    ssize_t nPos;
    u_char src[512] = {0};
    u_char dst[512] = {0};
    //keyInt = mergeArrayToInt(key);

    keyInt = 2;
    int i;
    for (i = 0; i < buf_size; i++) {
        //buf_pos[i] = buf_pos[i] ^ keyInt;
        src[i] = buf_pos[i] ^ keyInt;
    }

    ngx_buf_t *tmp_buf;
    ssize_t nSize = b->last - (buf_pos + buf_size);
    tmp_buf = ngx_create_temp_buf(r->pool, nSize);
    ngx_memcpy(tmp_buf->pos, buf_pos + buf_size, nSize);
    tmp_buf->last = tmp_buf->pos + nSize;

    nPos = ngx_strlen(src);
    //int cLen = 0;
    //TODO:base64 encode
    base64_encode(src, (char*)dst);
    //base64_encode((char*)src, nPos, (char*)dst, &cLen);
    //ByteToHexStr(src, dst, nPos);
    //Hex2Str(src, dst, ngx_strlen(src));
    
    nPos = b->last - buf_pos;
    ngx_memzero(buf_pos, nPos);
    //nPos = ngx_strlen(dst);
    nPos = buf_size;
    ngx_memcpy(buf_pos, dst, nPos);
    ngx_memcpy(buf_pos + nPos, tmp_buf->pos, nSize);
    nPos = ngx_strlen(dst) + nSize;
    b->last = buf_pos + nPos;
    nPos = ngx_strlen(b->last);
    ngx_memzero(b->last, nPos);
*/

    /*
    ssize_t tmp_size;
    if ((tmp_size = ngx_strlen(dst)) > buf_size) {
        *count = tmp_size - buf_size;
        *flag = 1;

    } else if ((tmp_size = ngx_strlen(dst)) < buf_size) {
        *count = buf_size - tmp_size;
        *flag = -1;
...
/*
    ...
    *flag = 0;
    ...
    ngx_pfree(r->pool, tmp_buf->start);
    tmp_buf->last = tmp_buf->pos;
    tmp_buf = NULL;
...
static ngx_int_t ngx_http_myproxy_encrypt_url(ngx_event_pipe_t *p, u_char *buf_pos, ssize_t buf_size, ngx_buf_t *b, ssize_t *count, ngx_int_t *flag)
{
    ...
    u_char key[20] = {0};
    ngx_memcpy(key, p->downstream->addr_text.data, p->downstream->addr_text.len);

    //TODO:
    if (ngx_myproxy_des_encrypt(p, key, buf_pos, buf_size, b, count, flag) != NGX_OK) {
        return NGX_ERROR;
    }
...
/*
static ngx_int_t 
ngx_http_myproxy_filter_url(ngx_event_pipe_t *p, ngx_buf_t *buf, ngx_int_t *size)
{
    ngx_buf_t *b;
            ...
            if ((q_pos = (u_char*)ngx_strstr(pos + position, url_tags[i])) != NULL) {
                ...
                while (j < 5) {
                    if ((k_pos = (u_char*)ngx_strnstr(o_pos, url_key[j], nLen)) != NULL) {
                        ...
                        //TODO:
                        if (ngx_http_myproxy_encrypt_url(p, t_pos, nSize, buf, &nCount, &iFlag) != NGX_OK) {
                            ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                                  "xor encrypt error");
                            return NGX_ERROR;
                        }
...
ngx_http_proxy_copy_filter(ngx_event_pipe_t *p, ngx_buf_t *buf)
{
    ...
    /*
    rSize = 0;
    rc = ngx_http_myproxy_filter_url(p, buf, &rSize);
    if (rc != NGX_OK) {
        return rc;
    }
    */
    ...
    if (p->length < 0 && rSize != 0) {
        ...
        for ( ;; ) {
            ...
            if (ngx_strncasecmp(header[i].key.data, (u_char *) "Content-Length", ngx_strlen("Content-Length")) == 0) {
                ...
                u_char tmp_buf[20] = {0};
                //ngx_snprintf(tmp_buf, url_size + 4, "url:%s", r->unparsed_uri.data);
                length += rSize;
                ngx_sprintf(tmp_buf, "%d", length);
                ngx_memzero(header[i].value.data, header[i].value.len);
                ngx_memcpy(header[i].value.data, tmp_buf, ngx_strlen(tmp_buf));
                header[i].value.len = ngx_strlen(tmp_buf);
...
static char* ngx_http_myproxy_create_url_file(ngx_str_t *url, ngx_str_t *name, ngx_log_t *log)
{
    //size_t len;
    ngx_uint_t create;
    ngx_file_t file;
    ...
    if(ngx_write_file(&file, url->data, url->len, 0) == NGX_ERROR) {
        return NGX_CONF_ERROR;
    }
...
ngx_http_proxy_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_http_myproxy_create_url_file(url, &peer_url_flag, cf->log);
    /*add by tu*/
