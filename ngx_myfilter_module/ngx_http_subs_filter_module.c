                                      ...
                                      ngx_buf_t *b, ngx_buf_t *dst)
{
    ...
    while (b->pos < b->last) {
        ...
        //TODO:
        if ((b->pos + pair->captures[1]) - (b->pos + pair->captures[0]) == 1 || 
                (b->pos + pair->captures[1]) - (b->pos + pair->captures[0]) == 2) {
            break;
        }

        int i = 0;
        for (; i < 6; i++) {
            if ((u_char*)ngx_strnstr(b->pos + pair->captures[0], key_format[i], (b->pos + pair->captures[1]) - (b->pos + pair->captures[0])) != NULL) {
                return count;
            }
        }
        ...
        */
        ...
        u_char url_out[1024];
        ngx_memset(url_out, '\0', 1024);
        size_t url_in_len = 0;
        int url_out_len = 0;
        url_in_len = (b->pos + pair->captures[1]) - (b->pos + pair->captures[0]);
        /*
        
        */

        base64_encode((char*)(b->pos + pair->captures[0]), (int)url_in_len, (char*)url_out, &url_out_len);

		if (buffer_append_string(dst, (u_char *)"10110010", ngx_strlen("10110010"),
									   r->pool) == NULL) {
				  return NGX_ERROR;
		  }

        char buffer[20] = {0};
        sprintf(buffer, "%d", url_out_len);
        if (url_out_len < 10) {
            if (buffer_append_string(dst, (u_char*)"000", ngx_strlen("000"),
                                        r->pool) == NULL) {
                    return NGX_ERROR;
            }

        } else if (url_out_len < 100 && url_out_len >= 10) {
            if (buffer_append_string(dst, (u_char*)"00", ngx_strlen("00"),
                                        r->pool) == NULL) {
                    return NGX_ERROR;
            }

        } else if (url_out_len < 1000 && url_out_len >= 100) {
            if (buffer_append_string(dst, (u_char*)"0", ngx_strlen("0"),
                                        r->pool) == NULL) {
                    return NGX_ERROR;
            }
        }

		if (buffer_append_string(dst, (u_char*)buffer, ngx_strlen(buffer),
									   r->pool) == NULL) {
				  return NGX_ERROR;
		  }

		if (buffer_append_string(dst, (u_char *)url_out, (size_t)url_out_len,
									   r->pool) == NULL) {
				  return NGX_ERROR;
		  }
